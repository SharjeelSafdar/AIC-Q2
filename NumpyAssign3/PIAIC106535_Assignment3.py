...
def function6():
    ...
    arr = np.zeros(10, dtype=np.int32) #wrtie your code here
    arr[4] = 10
    arr[7] = 20
...
def function8():
    ...
    x = np.full(shape=(2, 5), fill_value=6, dtype=np.uint32) #write your code here
...
def function9():
    ...
    a = np.arange(start=2, stop=102, step=2, dtype=np.uint32) # write your code here
...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = np.subtract(arr.T, brr) # write your code here 
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.vstack((arr, arr, arr)).T.flatten() #write your code here 
    ans = np.hstack((brr, arr, arr, arr))
...
def function16():
    ...
    x = np.array([[1], [2], [3]])
    y = np.array([[2], [3], [4]])
    ans = np.stack((x,y), axis=2) #write your code here 
